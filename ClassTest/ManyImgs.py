import cv2
import numpy as np
# 定义图片组合函数，第一个参数是缩放比例，第二个参数是需要显示的图片组成的元组或者列表
def ManyImgs(scale, imgarray):
    ...
    # 判断imgarray[0]的类型是否是list，如果是 list，则表明 imgarray 是一个元组，需要垂直显示
        ...
        for x in range(0, rows):
            for y in range(0, cols):
                ...
                if  len(imgarray[x][y].shape) == 2:
                    imgarray[x][y] = cv2.cvtColor(imgarray[x][y],cv2.COLOR_GRAY2BGR)#cv2.COLOR_GRAY2BGR
        # 创建一个空白画布，与第一张图片大小相同
